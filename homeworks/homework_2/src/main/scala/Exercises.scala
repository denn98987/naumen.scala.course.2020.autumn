...
object Exercises {
    ...
    def sumByFunc(leftVec0: Vector2D,
                  leftVec1: Vector2D,
                  lambda: (Vector2D, Vector2D) => Double,
                  rightVec0: Vector2D,
                  rightVec1: Vector2D) =
        lambda(leftVec0, leftVec1) + lambda(rightVec0, rightVec1)
    ...
    */
    ...
    def sortByHeavyweight(ballsArray: Map[String, (Int, Double)] = balls): Seq[String] =
        balls.toSeq.sortBy(sortFunc).map(_._1)
    def calcWeight(radius:Int, density:Double) = 4/3 * java.lang.Math.PI*pow(radius, 3)*density
    def sortFunc(param: (String, (Int, Double))) = calcWeight(param._2._1, param._2._2)
